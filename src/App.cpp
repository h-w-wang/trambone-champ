...
void App::Start() {
    ...
    // 🚀 初始化背景也套用動態縮放
    auto defaultBg = std::make_shared<Util::Image>(RESOURCE_DIR "/warmup-light.png");
    m_Background->SetDrawable(defaultBg);
    glm::vec2 initBgSize = defaultBg->GetSize();
    if (initBgSize.x > 0 && initBgSize.y > 0) {
        float sX = 1280.0f / initBgSize.x;
        float sY = 720.0f / initBgSize.y;
        float fScale = std::max(sX, sY);
        m_Background->m_Transform.scale = {fScale, fScale};
    } else {
    ...
    for (int i = 0; i <= 24; ++i) {
        ...
        if (i == 0 || i == 12 || i == 24) {
            line->m_Transform.scale = {30.0f, 0.09f};
        } else {
            line->m_Transform.scale = {30.0f, 0.02f};
        }
...
void App::PauseUpdate() {
    ...
    if (m_IsCountingDown) {
        ...
        if (remain > 0) {
            if (!s_CountdownText) {
                s_CountdownText = std::make_shared<Util::GameObject>();
                s_CountdownText->SetZIndex(99.0f);
                s_CountdownBG = std::make_shared<Util::GameObject>();
                s_CountdownBG->SetDrawable(std::make_shared<Util::Image>(RESOURCE_DIR "/note-dot.png"));
                s_CountdownBG->SetZIndex(98.0f);
            }
            try {
                s_CountdownText->SetDrawable(std::make_shared<Util::Text>(RESOURCE_DIR "/font.ttc", 150, std::to_string(remain), SDL_Color{255, 255, 0, 255}));
            } catch(...) {}

            s_CountdownBG->m_Transform.scale = {3.0f, 3.0f};
            s_CountdownBG->Draw();
            s_CountdownText->Draw();
            return;
        } else {
            m_IsCountingDown = false;
